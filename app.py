...
st.write("בחר נכס, טווח זמן וסכום השקעה - וקבל תחזית עם גרף, חיווי וניתוח מגמה.")
...
def get_trend(data):
    ...
    if pd.isna(data['SMA5'].iloc[-1]) or pd.isna(data['SMA20'].iloc[-1]):
        return "נתונים לא מספיקים לניתוח מגמה", "neutral"
...
if st.button("קבל תחזית"):
    try:
        ...
        trend, signal = get_trend(data)
        predicted_price = current_price * (1.02 if signal == "buy" else 0.98)
